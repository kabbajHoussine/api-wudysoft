...
class WudysoftAPI {
  ...
  async checkMessagesSoro2(email) {
    try {
      ...
      console.error(`[ERROR] Gagal dalam 'WudysoftAPI.checkMessagesSoro2' untuk email ${email}: ${error.message}`);
      return null;
    }
  }
...
class Soro2AIAPI {
  constructor() {
    ...
    const commonHeaders = {
      ...
      referer: "https://soro2.ai/ai-video/sora2",
  ...
  async _performRegistration() {
    ...
    await this.cookieJar.setCookie(`NEXT_LOCALE=en; Domain=.soro2.ai; Path=/; Secure; SameSite=Lax`, "https://soro2.ai");
    ...
    const callbackResponse = await this.api.post(`${this.config.endpoints.auth.callback}?`, `email=${encodeURIComponent(email)}&code=${verificationCode}&redirect=false&csrfToken=${encodeURIComponent(csrfToken)}&callbackUrl=https%3A%2F%2Fsoro2.ai%2Fai-video%2Fsora2`, {
      headers: {
    ...
    const cookies = await this.cookieJar.getCookies("https://soro2.ai");
  ...
  async register() {
    try {
      console.log("Proses: Mendaftarkan sesi Soro2 AI baru...");
      const sessionData = await this._performRegistration();
      const sessionToSave = JSON.stringify(sessionData);
      const sessionTitle = `soro2ai-session-${this._random()}`;
      ...
      console.log(`-> Sesi Soro2 AI baru berhasil didaftarkan. Kunci Anda: ${newKey}`);
      return {
        key: newKey,
      ...
      console.error(`Proses registrasi Soro2 AI gagal: ${error.message}`);
      throw error;
    }
  }
  async _ensureValidSession({
    ...
    await this.cookieJar.setCookie(`__Secure-authjs.session-token=${sessionData.sessionToken}; Domain=.soro2.ai; Path=/; Secure; SameSite=Lax`, "https://soro2.ai");
    await this.cookieJar.setCookie(`NEXT_LOCALE=en; Domain=.soro2.ai; Path=/; Secure; SameSite=Lax`, "https://soro2.ai");
    return {
  ...
  async video({
    ...
    try {
      ...
      console.log(`Proses: Membuat video dengan model Soro2 AI...`);
      const payload = {
        prompt: prompt,
  ...
  async list_key() {
    try {
      console.log("Proses: Mengambil daftar semua kunci sesi Soro2 AI...");
      const allPastes = await this.wudysoft.listPastes();
      return allPastes.filter(paste => paste.title && paste.title.startsWith("soro2ai-session-")).map(paste => paste.key);
    } catch (error) {
